...
## ADR-M20-001 — Explicit truth on contextual Relation assertions

M20 keeps positive and negative pairwise knowledge in the existing Entity-owned `relation_assertions` collection. An optional boolean `truth` field defaults to `true` for backward-compatible positive assertions; `truth: false` is an evidence-backed known negative for the exact source, controlled relation key, target, and normalized context. If no applicable assertion exists, lookup remains ABSENT/UNKNOWN. The most-specific applicable assertion set determines the result, and mixed truth values at equal specificity are a deterministic semantic error rather than a source-order choice.

Exact duplicate assertions are rejected. A positive and negative assertion for the same source/key/target/context is rejected as contradictory. Relation-family target cardinality counts only positive assertions: `metal.product_cation` and `ion.elemental_substance` still allow one positive target per context, while any number of distinct evidence-backed rejected targets may be recorded without redefining positive cardinality. Target-producing Relation resolution exposes only positive assertions.

The bounded chemistry proof authors only `Cu(s) metal.displaces_cation Zn2+ = false` in aqueous context. Its OpenStax-backed assertion makes the M19 displacement predicate known FALSE for Cu + aqueous ZnSO4, while an unsupported Cu/Mg2+ pair remains UNKNOWN and the six existing Zn/Mg positives remain TRUE. This is pairwise authored knowledge, not an activity-series ordering, transitive comparison, electrode-potential runtime, or general redox engine.
...
## ADR-M24-001 — Legacy Reactions reconcile only to existing canonical truth

M24 adds `migration/legacy_reaction.py` as an offline orchestration owner while retaining `migration/legacy_identity.py` as the only participant-identity owner. A fixed 18-record cohort at legacy revision `a6311150436038ca06fa7b9d05de39da9e1de815` spans neutralization, precipitation, carbonate/hydrogen-carbonate and sulfite gas evolution, ammonium/base chemistry, Zn/CuSO4 displacement, and CaCO3/NaHCO3 thermal decomposition. Sixteen records map to exactly one existing canonical Reaction; calcium carbonate + HCl has no canonical counterpart, and catalytic ammonia oxidation needs unsupported catalyst semantics, so both are skipped. Canonical creation is prohibited and `created_canonical` is zero.

The migration layer resolves every participant through approved identity semantics, then validates roles, positive integral coefficients, duplicate merging, common-factor reduction, exact atom/charge conservation, explicit phase mapping, and the already-controlled context values. It reuses the canonical Reaction signature and condition comparison, never reverses direction, never infers aqueous phase, and fails closed on missing/conflicting conditions, unsupported strings, unresolved identities, invalid shapes, conservation failure, or multiple compatible canonical matches. A narrowly gated elemental-Substance identity profile supports explicit single-referent Zn/Cu lookup through canonical elemental formula keys without changing M22/M23 defaults.

Legacy molecular and ionic equations are review evidence only. Net-ionic comparison may report `compatible`, `unavailable`, or `unsupported_inconsistent`, but cannot select, create, or rewrite a canonical ReactionForm; canonical forms remain derived from canonical Reaction and speciation owners. The deterministic report is operational provenance and migration does not mutate canonical source or become a compiler dependency. Missing canonical Reactions, catalyst/light or richer condition vocabulary, and broader polyatomic-ion identity remain separate future architecture pressures. Compatibility coordinates remain source schema `3.7.0`, Rule DSL `1.4.0`, RulePlan `1.4.0`, and artifact format `1.5.0`.

## ADR-M25-001 — Full-corpus audit closes migration machinery without bulk import

M25 extends `migration/legacy_reaction.py`, rather than creating a second engine, to audit all 152 Reactions declared by the fixed legacy manifest. The report retains the closed M24 dispositions and adds one deterministic future-action bucket to every non-mapped record. Final counts are 20 mapped, 123 unsupported, nine rejected reversible records, zero ambiguous, and zero automatically created; buckets are 88 identity, 25 context, ten ordinary curation, nine chemistry-architecture, and zero invalid-legacy gaps. Reversible records are architecture gaps because the current model does not own equilibrium direction, even though their migration disposition remains fail-closed rejection.

Layer A is strictly read-only. Layer B admits only `rxn_m25_hcl_caco3_gas_evolution`, supported by existing authoritative carbonate/acid and identity evidence rather than legacy provenance. Its exact solid/aqueous/gas/liquid participants conserve atoms and charge, and existing canonical speciation derives complete and net ionic forms. No Evidence, Rule, schema, compiler branch, or condition vocabulary is added. The soluble-carbonate Rule remains gated by aqueous phase, solubility, and complete dissociation, so it does not infer solid CaCO3; canonical Reaction coverage does not imply executable Rule coverage.

The remaining records are explicitly classified and require ordinary evidence curation or separately designed identity, controlled-context, reversible/equilibrium, redox, or competing-pathway work. Polyatomic legacy ions remain diagnostic-only blockers where molecular reconciliation already succeeds. Migration reports are deterministic operational provenance, and compiler runtime remains independent of `migration/**`. These facts satisfy the M6 exit criteria: `M6_EXIT_READY = true`, with no M26 required. Compatibility coordinates remain `3.7.0`, `1.4.0`, `1.4.0`, and `1.5.0`.
