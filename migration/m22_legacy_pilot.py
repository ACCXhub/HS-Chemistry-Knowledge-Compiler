...
def reconcile_record(
    ...
    try:
        ...
        candidates, facts = _verified_candidates(record_family, record, entities)
...
def run_pilot(repo_root: Path, legacy_root: Path, cohort_path: Path) -> dict[str, Any]:
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(description="Run the bounded M22 legacy migration pilot")
...
def main(argv: list[str] | None = None) -> int:
    ...
    output = args.output or repo_root / "migration" / "reports" / "m22_pilot_report.json"
    report = run_pilot(repo_root, args.legacy_root.resolve(), cohort_path.resolve())
