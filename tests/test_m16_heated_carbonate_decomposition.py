from __future__ import annotations

import json
from pathlib import Path

from compiler.build import ARTIFACT_FORMAT_VERSION, artifact_versions, compile_repository
from compiler.engine import infer_case
from compiler.rules import RULE_DSL_VERSION, RULE_PLAN_VERSION, compile_rules
from compiler.source import SOURCE_SCHEMA_VERSION, load_cases, load_knowledge


ROOT = Path(__file__).resolve().parents[1]


def _case(case_id: str, substance_id: str = "ent_substance_caco3", *, phase: str = "solid", temperature: str | None = "heated") -> dict:
    context = {}
    if temperature is not None:
        context["temperature_regime"] = temperature
    return {
        "id": case_id,
        "reactants": [{"target_id": substance_id, "phase": phase}],
        "context": context,
    }


def _normalized(participants: list[dict]) -> list[tuple[str, str, str, int]]:
    return sorted(
        (
            item["role"],
            item["target_id"],
            item["phase"],
            item["coefficient"]["numerator"],
        )
        for item in participants
    )


def test_m16_source_contract_adds_heated_without_runtime_coordinate_drift() -> None:
    schema = json.loads((ROOT / "schemas" / "knowledge-record.schema.json").read_text(encoding="utf-8"))

    assert SOURCE_SCHEMA_VERSION == "3.5.0"
    assert schema["x-source-schema-version"] == "3.5.0"
    temperature_values = schema["$defs"]["reactionCondition"]["allOf"][1]["then"]["properties"]["value"]["enum"]
    assert temperature_values == ["ambient", "warmed", "heated"]
    assert RULE_DSL_VERSION == "1.3.0"
    assert RULE_PLAN_VERSION == "1.3.0"
    assert ARTIFACT_FORMAT_VERSION == "1.4.0"
    assert artifact_versions() == {
        "source_schema": "3.5.0",
        "rule_dsl": "1.3.0",
        "rule_plan": "1.3.0",
        "artifact_format": "1.4.0",
    }


def test_m16_caco3_heated_infers_exact_canonical_decomposition_deterministically() -> None:
    kb = load_knowledge(ROOT)
    plans = compile_rules(kb)
    case = _case("m16_caco3_heated")

    first = infer_case(kb, plans, case)
    second = infer_case(kb, plans, case)

    assert first == second
    assert first["status"] == "inferred"
    assert first["rule_id"] == "rule_m16_caco3_heated_decomposition"
    assert first["canonical_match"]["state"] == "exact"
    assert first["canonical_match"]["reaction_ids"] == ["rxn_m16_caco3_heated_decomposition"]
    assert first["canonical_match"]["condition_evidence_ids"] == ["ev_m16_caco3_heated_decomposition"]
    assert first["provenance"]["condition_evidence_ids"] == ["ev_m16_caco3_heated_decomposition"]
    assert first["validation"] == {"atoms": True, "charge": True}
    assert _normalized(first["participants"]) == [
        ("product", "ent_substance_cao", "solid", 1),
        ("product", "ent_substance_co2", "gas", 1),
        ("reactant", "ent_substance_caco3", "solid", 1),
    ]
    assert first["provenance"]["evidence_ids"]


def test_m16_fixture_matrix_preserves_missing_unknown_and_rejects_other_conditions() -> None:
    kb = load_knowledge(ROOT)
    plans = compile_rules(kb)
    cases = {case["id"]: case for case in load_cases(ROOT)}
    expected = {
        "case_m16_caco3_heated": ("inferred", "rule_m16_caco3_heated_decomposition"),
        "case_m16_caco3_missing_temperature": ("indeterminate", None),
        "case_m16_caco3_ambient": ("no_match", None),
        "case_m16_caco3_warmed": ("no_match", None),
        "case_m16_caco3_wrong_phase": ("no_match", None),
        "case_m16_mgco3_heated_deferred": ("no_match", None),
    }

    for case_id, (status, rule_id) in expected.items():
        result = infer_case(kb, plans, cases[case_id])
        assert result["status"] == status, case_id
        if rule_id is None:
            assert "candidate_key" not in result, case_id
        else:
            assert result["rule_id"] == rule_id, case_id

    missing = infer_case(kb, plans, cases["case_m16_caco3_missing_temperature"])
    ambient = infer_case(kb, plans, cases["case_m16_caco3_ambient"])
    warmed = infer_case(kb, plans, cases["case_m16_caco3_warmed"])
    assert any(
        item.get("rule_id") == "rule_m16_caco3_heated_decomposition"
        and item.get("key") == "temperature_regime"
        and item.get("truth") == "UNKNOWN"
        for item in missing["proof_trace"]
    )
    for result in (ambient, warmed):
        assert any(
            item.get("rule_id") == "rule_m16_caco3_heated_decomposition"
            and item.get("key") == "temperature_regime"
            and item.get("truth") == "FALSE"
            for item in result["proof_trace"]
        )


def test_m16_rule_is_exact_and_does_not_define_dynamic_carbonate_products() -> None:
    kb = load_knowledge(ROOT)
    plan = next(plan for plan in compile_rules(kb) if plan.rule_id == "rule_m16_caco3_heated_decomposition")

    assert len(plan.patterns) == 1
    assert plan.patterns[0].target_id == "ent_substance_caco3"
    assert plan.patterns[0].phase == "solid"
    assert [(product.target_id, product.phase) for product in plan.products] == [
        ("ent_substance_cao", "solid"),
        ("ent_substance_co2", "gas"),
    ]


def test_m16_thermal_reaction_has_no_ionic_reaction_forms(tmp_path: Path) -> None:
    kb = load_knowledge(ROOT)
    reaction = kb.reactions["rxn_m16_caco3_heated_decomposition"]
    assert reaction["forms"] == []

    compile_repository(ROOT, tmp_path / "compile", "m16-test")
    derived = json.loads((tmp_path / "compile" / "derived-reaction-forms.json").read_text(encoding="utf-8"))
    assert all(item["reaction_id"] != reaction["id"] for item in derived["forms"])


def test_m16_cao_identity_is_unique_and_evidence_backed() -> None:
    kb = load_knowledge(ROOT)

    assert kb.semantic_key_candidates("formula.unit", "CaO") == ("ent_substance_cao",)
    cao = kb.entities["ent_substance_cao"]
    assert cao["evidence_ids"] == ["ev_m16_caco3_heated_decomposition"]
    assert cao["payload"]["composition"] == {
        "components": [
            {"element_id": "ent_element_ca", "count": 1},
            {"element_id": "ent_element_o", "count": 1},
        ],
        "net_charge": 0,
    }


def test_m16_teaching_view_reuses_existing_view_without_claiming_ionic_equations() -> None:
    kb = load_knowledge(ROOT)
    assert tuple(kb.teaching_views) == ("view_f3b_hs_aqueous_core",)
    nodes = {
        node["path_key"]: set(node.get("members", []))
        for node in kb.teaching_views["view_f3b_hs_aqueous_core"]["nodes"]
    }

    reaction_id = "rxn_m16_caco3_heated_decomposition"
    assert reaction_id in nodes["D03/reaction-types/decomposition"]
    assert reaction_id not in nodes["D06/notation/ionic-equations"]
    assert {"ent_substance_caco3", "ent_substance_cao"} <= nodes[
        "D10/elements-and-compounds/representative-elements"
    ]


def test_m16_preserves_m7_warmed_m11_ambient_and_m15_precipitation() -> None:
    kb = load_knowledge(ROOT)
    plans = compile_rules(kb)
    cases = {case["id"]: case for case in load_cases(ROOT)}
    expected = {
        "case_m7_nh4cl_naoh": "rule_m7_ammonium_strong_base_gas_evolution",
        "case_m11_na_water": "rule_m11_water_reactive_metal_hydrogen",
        "case_m15_cuso4_naoh": "rule_m15_hydroxide_precipitation",
    }

    for case_id, rule_id in expected.items():
        result = infer_case(kb, plans, cases[case_id])
        assert result["status"] == "inferred", case_id
        assert result["rule_id"] == rule_id, case_id
        assert result["canonical_match"]["state"] == "exact", case_id
